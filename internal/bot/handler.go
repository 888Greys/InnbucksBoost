...
func (b *Bot) handleUpdate(ctx context.Context, update tgbotapi.Update) {
	...
	case msg.Text == "💬 Support":
		b.sendText(chatID, "💬 *InnBoost Support*\n\nDM us directly: @innbuckshelp\n\n⏱ Response time: within 1 hour.\n\n_For order issues include your Order # in the message._")
...
func (b *Bot) sendCategoryMenu(chatID int64) {
	msg := tgbotapi.NewMessage(chatID, "🚀 *InnBoost*\n\nChoose a platform to get started:")
...
func (b *Bot) editToCategoryMenu(chatID int64, msgID int) {
	edit := tgbotapi.NewEditMessageText(chatID, msgID, "🚀 *InnBoost*\n\nChoose a platform to get started:")
	edit.ParseMode = "Markdown"
	kb := categoryKeyboard()
	edit.ReplyMarkup = &kb
	b.api.Send(edit)
	...
	if info.IsPrivate {
		...
		edit := tgbotapi.NewEditMessageText(chatID, scanMsgID,
			fmt.Sprintf("🔒 *Private Account Detected*\n\n"+
				...
				"InnBoost requires a *public* account to deliver followers.\n\n"+
...
func totalFollowersInPackage(pkg Package) int {
	followerSvcs := map[int]bool{5760: true, 5440: true}
	total := 0
	for _, comp := range pkg.Components {
		if followerSvcs[comp.ServiceID] {
			total += comp.Quantity
		}
	}
	return total
...
func (b *Bot) sendHowItWorks(chatID int64) {
	b.sendText(chatID,
		"❓ *How InnBoost Works*\n\n"+
			...
			"InnBoost verifies your public profile and shows you exactly what account will be boosted.\n\n"+
			"*3️⃣ Pay with M-Pesa*\n"+
			...
			"InnBoost places your order instantly. Followers and views start arriving within minutes to a few hours.\n\n"+
			...
			"Questions? Contact @innbuckshelp",
	)
}

func (b *Bot) sendAccountSafety(chatID int64) {
	b.sendText(chatID,
		"🛡️ *InnBoost Safety — Anti-Ban Protection*\n\n"+
			"Your account safety is InnBoost's #1 priority. Here's how we protect you:\n\n"+
			"✅ *Organic-Speed Delivery*\n"+
			"InnBoost never dumps thousands of followers at once. Delivery happens at a pace that looks 100% natural to platform algorithms.\n\n"+
			...
			"All InnBoost services use real-looking, aged accounts — not obvious bots.\n\n"+
			"✅ *30-Day Refill Guarantee*\n"+
			"If any followers drop within 30 days, InnBoost tops them back up automatically at no extra cost.\n\n"+
...
func (b *Bot) sendReferralInfo(ctx context.Context, chatID, telegramID int64) {
	...
	b.sendText(chatID, fmt.Sprintf(
		"🤝 *Refer & Earn*\n\n"+
			...
			"_Credits can be used toward your next order. DM @innbuckshelp to redeem._",
		link, balance,
	))
}
	...
	if clientTgID > 0 {
		b.sendText(clientTgID, fmt.Sprintf(
			"⚠️ *Order #%d Update*\n\n"+
				...
				"Please DM @innbuckshelp for assistance.",
			orderID,
		))
	}
